...
import com.thinkgem.jeesite.modules.user.service.UserLccService;
 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/user/userUserinfoExPmtc")
public class UserUserinfoExPmtcController extends BaseController {
    ...
    public String userCharge(UserChargeValidate chargeValidate, Model model, RedirectAttributes redirectAttributes) throws ValidationException {
        ...
        try {
            if(chargeValidate.getChargeType().equals(EnumUtil.MoneyType.money.toString())){
                ...
                userUserinfoService.updateUserMoney(chargeValidate.getUserName(), new BigDecimal(chargeValidate.getChargeMoney()), chargeValidate.getCommt()+"-配额" + UserUtils.getUser().getLoginName(), EnumUtil.MoneyChangeType.charget);
            }
            else if(chargeValidate.getChargeType().equals(EnumUtil.MoneyType.score.toString())){

                userUserinfoService.updateUserScore(chargeValidate.getUserName(), new BigDecimal(chargeValidate.getChargeMoney()), chargeValidate.getCommt()+"-配额" + UserUtils.getUser().getLoginName(), EnumUtil.MoneyChangeType.charget);
            }
            else if(chargeValidate.getChargeType().equals(EnumUtil.MoneyType.coin.toString())){

                userUserinfoService.updateUserCoin(chargeValidate.getUserName(), new BigDecimal(chargeValidate.getChargeMoney()), chargeValidate.getCommt()+"-配额" + UserUtils.getUser().getLoginName(), EnumUtil.MoneyChangeType.charget);
            }
            else if(chargeValidate.getChargeType().equals(EnumUtil.MoneyType.money2.toString())){

                userLccService.updateUserOtherMoney(chargeValidate.getUserName(), new BigDecimal(chargeValidate.getChargeMoney()), EnumUtil.MoneyType.money2, chargeValidate.getCommt()+"-配额" + UserUtils.getUser().getLoginName(), EnumUtil.MoneyChangeType.charget);
//                UserUserinfo userinfo = userUserinfoService.getByName(userName);
//                if (userinfo != null) {
////                if(userinfo.getActiveStatus().equals(EnumUtil.YesNO.NO.toString())){
                if(new BigDecimal(chargeValidate.getChargeMoney()).compareTo(BigDecimal.ZERO)>0){
                    userLccService.activeUser(chargeValidate.getUserName(), new BigDecimal(chargeValidate.getChargeMoney()));
                }
//                }
                //}
     ...
     * @return
     */
    @RequestMapping(value = "/findTeamUserList")
    public String findTeamUserList(UserUserinfo userUserinfo , HttpServletRequest request, HttpServletResponse response, Model model){
        model.addAttribute("userLevels",userLevelService.findList(new UserLevel()));
        Page<UserUserinfo> page = userUserinfoService.findPage(new Page<UserUserinfo>(request,response),userUserinfo);
        model.addAttribute("page",page);
        model.addAttribute("peopleNum",page.getList().size());
        return "modules/user/userTeamAward";
    }

    @RequestMapping(value = "/teamAward")
    public String teamAward(UserUserinfo userUserinfo , HttpServletRequest request,  RedirectAttributes redirectAttributes){
        String tempMoney = request.getParameter("sumMoney");
        String message = "奖励发放成功";
        try {
            BigDecimal sumMoney = new BigDecimal(tempMoney);
            userLccService.putTeamAward(userUserinfo,sumMoney);
        }catch (ValidationException e){
        ...
        return "redirect:" + Global.getAdminPath() + "/user/userUserinfoExPmtc/findTeamUserList?userLevelId="+userUserinfo.getUserLevelId();
    }
     ...
     * @return
     */
    @RequestMapping(value = "changeUserParent", method = RequestMethod.GET)
    public String changeUserParent(HttpServletRequest request, HttpServletResponse response,Model model) {
        String userName = request.getParameter("userName");
        model.addAttribute("changeName",userName);
        return "modules/user/changeUserParent";
    }
     ...
     * @return
     */
    @RequestMapping(value = "changeUserParentDo", method = RequestMethod.POST)
    public String changeUserParentDo(String changeUser,String parentName,HttpServletRequest request,HttpServletResponse response,  RedirectAttributes redirectAttributes,Model model) {

        if(StringUtils2.isBlank(changeUser) || StringUtils2.isBlank(parentName)){
            addMessage(model,"失败:请选择要修改的用户或者他的父级用户");
            return changeUserParent(request,response,model);
        }
        String message = "修改上级成功";
        try {
            userLccService.updateParentList(changeUser,parentName);
        }catch (ValidationException e){
            message = "修改上级失败:"+e.getMessage();
        }catch (Exception e){
            message = "修改上级失败";
        }
