...
public class GenTableService extends BaseService {
	...
	public boolean checkTableName(String tableName){
		if (StringUtils2.isBlank(tableName)){
			return true;
		}
	...
	public GenTable getTableFormDb(GenTable genTable){
		// 如果有表名，则获取物理表
		if (StringUtils2.isNotBlank(genTable.getName())){
			
			List<GenTable> list = genDataBaseDictDao.findTableList(genTable);
			if (list.size() > 0){
				
				// 如果是新增，初始化表属性
				if (StringUtils2.isBlank(genTable.getId())){
					genTable = list.get(0);
					// 设置字段说明
					if (StringUtils2.isBlank(genTable.getComments())){
						genTable.setComments(genTable.getName());
					}
					genTable.setClassName(StringUtils2.toCapitalizeCamelCase(genTable.getName()));
				}
	...
	public void save(GenTable genTable) throws ValidationException {
		if (StringUtils2.isBlank(genTable.getId())){
			genTable.preInsert();
		...
		for (GenTableColumn column : genTable.getColumnList()){
			...
			if (StringUtils2.isBlank(column.getId())){
				column.preInsert();
