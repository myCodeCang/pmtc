...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		if (token.getUserType().equals(Global.USER_ADMIN)) {
			// 校验登录验证码
			if (UserUtils.isValidateCodeLogin(token.getUsername(), false,
					false)) {
			...
			if (user != null) {
				...
				byte[] salt = Encodes.decodeHex(user.getPassword().substring(0,
						16));
				return new SimpleAuthenticationInfo(new Principal(user,
						token.isMobileLogin(),token.getUserType()), user.getPassword()
	...
	public UserUserinfoService getUserInfoService() {
		if (userUserinfoService == null) {
			userUserinfoService = SpringContextHolder.getBean(UserUserinfoService.class);
		}
