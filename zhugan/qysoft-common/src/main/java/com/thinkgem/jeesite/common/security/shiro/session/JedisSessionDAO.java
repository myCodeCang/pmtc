 ...
 * @version 2014-7-20
 */
public class JedisSessionDAO extends AbstractSessionDAO implements SessionDAO2 {
	...
	public void update(Session session) throws UnknownSessionException {
		...
		if (request != null){
			...
			// 如果是视图文件，则不更新SESSION
			if (StringUtils2.startsWith(uri, Global.getConfig("web.view.prefix"))
					&& StringUtils2.endsWith(uri, Global.getConfig("web.view.suffix"))){
				return;
			}
			// 手动控制不更新SESSION
		...
		try {
			...
			String principalId = pc != null ? pc.getPrimaryPrincipal().toString() : StringUtils2.EMPTY;
	...
	public Collection<Session> getActiveSessions(boolean includeLeave, Object principal, Session filterSession){
		...
		try {
			...
			for (Map.Entry<String, String> e : map.entrySet()){
				if (StringUtils2.isNotBlank(e.getKey()) && StringUtils2.isNotBlank(e.getValue())){
					
					String[] ss = StringUtils2.split(e.getValue(), "|");
					if (ss != null && ss.length == 3){// jedis.exists(sessionKeyPrefix + e.getKey())){
						...
						try{
							...
							if (includeLeave || DateUtils2.pastMinutes(session.getLastAccessTime()) <= 3){
								isActiveSession = true;
							}
							// 符合登陆者条件。
							if (principal != null){
								PrincipalCollection pc = (PrincipalCollection)session.getAttribute(DefaultSubjectContext.PRINCIPALS_SESSION_KEY);
								if (principal.toString().equals(pc != null ? pc.getPrimaryPrincipal().toString() : StringUtils2.EMPTY)){
									isActiveSession = true;
								}
							}
							// 过滤掉的SESSION
				...
				// 存储的SESSION无Value
				else if (StringUtils2.isNotBlank(e.getKey())){
					jedis.hdel(sessionKeyPrefix, e.getKey());
				}
			}
