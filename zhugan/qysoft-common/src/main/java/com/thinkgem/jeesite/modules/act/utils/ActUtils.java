...
import com.thinkgem.jeesite.common.utils.ObjectUtils2;
...
//				if(StringUtils2.startsWith(field.getName(), "PD_")){
//					try{
...
//		String url = procDefMap.get(StringUtils2.split(procId, ":")[0]);
...
//	}
	...
	public static Map<String, Object> getMobileEntity(Object entity,String spiltType){
			...
			for (Method m : entity.getClass().getMethods()){
				if (m.getAnnotation(JsonIgnore.class) == null && m.getAnnotation(JsonBackReference.class) == null && m.getName().startsWith("get")){
					...
					if (m.getName().equals("getAct")){
						...
						map.put("taskId", ObjectUtils2.toString(m.invoke(act, new Object[]{}), ""));
					}else{
						field.add(StringUtils2.uncapitalize(m.getName().substring(3)));
						value.add(ObjectUtils2.toString(m.invoke(entity, new Object[]{}), ""));
					}
				}
			}
		}catch (Exception e) {
		...
		map.put("beanTitles", StringUtils2.join(field, spiltType));
		map.put("beanInfos", StringUtils2.join(value, spiltType));
		map.put("chineseNames", StringUtils2.join(chinesName, spiltType));
	...
	public static String getFormUrl(String formKey, Act act){
		...
		if (StringUtils2.isBlank(formServerUrl)){
	...
	public static UserEntity toActivitiUser(User user){
		...
		userEntity.setLastName(StringUtils2.EMPTY);
