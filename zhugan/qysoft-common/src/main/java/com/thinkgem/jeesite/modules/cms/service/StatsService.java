...
public class StatsService extends BaseService {
	...
	public List<Category> article(Map<String, Object> paramMap) {
		...
		Date beginDate = DateUtils2.parseDate(paramMap.get("beginDate"));
		if (beginDate == null){
			beginDate = DateUtils2.setDays(new Date(), 1);
			paramMap.put("beginDate", DateUtils2.formatDate(beginDate, "yyyy-MM-dd"));
		}
		category.setBeginDate(beginDate);
		Date endDate = DateUtils2.parseDate(paramMap.get("endDate"));
		if (endDate == null){
			endDate = DateUtils2.addDays(DateUtils2.addMonths(beginDate, 1), -1);
			paramMap.put("endDate", DateUtils2.formatDate(endDate, "yyyy-MM-dd"));
		}
