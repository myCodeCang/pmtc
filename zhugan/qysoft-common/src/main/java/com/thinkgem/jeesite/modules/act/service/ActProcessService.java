...
public class ActProcessService extends BaseService {
	...
	public Page<ProcessInstance> runningList(Page<ProcessInstance> page, String procInsId, String procDefKey) {
	    ...
	    if (StringUtils2.isNotBlank(procInsId)){
		    processInstanceQuery.processInstanceId(procInsId);
	    }
	    
	    if (StringUtils2.isNotBlank(procDefKey)){
	...
	public InputStream resourceRead(String procDefId, String proInsId, String resType) throws Exception {
		...
		if (StringUtils2.isBlank(procDefId)){
	...
	public String deploy(String exportDir, String category, MultipartFile file) {
		...
		try {
			...
			if (processDefinitionQuery != null) {
				ProcessDefinitionQuery deploymentId = processDefinitionQuery.deploymentId(deployment.getId());
				if (deploymentId != null) {
					list = deploymentId.list();
				}
			}
...
//					ActUtils.exportDiagramToFile(repositoryService, processDefinition, exportDir);
				...
				if (null != processDefinition.getId()) {
					messBuilder.append("部署成功，流程ID=").append(processDefinition.getId()).append("<br/>");
	...
	public List<String> exportDiagrams(String exportDir) throws IOException {
		...
		for (ProcessDefinition processDefinition : list) {
			...
			if (!diagramDirFile.exists()) {
				boolean mkdirs = diagramDirFile.mkdirs();
				if (!mkdirs) {
					logger.error("创建文件失败");
					continue;
				}
