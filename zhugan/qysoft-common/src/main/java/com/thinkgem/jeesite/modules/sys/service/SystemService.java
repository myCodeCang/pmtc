...
public class SystemService extends BaseService implements InitializingBean {
	...
	public SessionDAO2 getSessionDao() {
		return sessionDao;
	}
	...
	public void saveUser(User user) throws ValidationException {
		if (StringUtils2.isBlank(user.getId())){
			user.preInsert();
			userDao.insert(user);
		}else{
			// 清除原用户机构用户缓存
		...
		if (StringUtils2.isNotBlank(user.getId())){
			// 更新用户与角色关联
...
//			systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public void updateUserInfo(User user) throws ValidationException {
		user.preUpdate();
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public void updateUserLoginInfo(User user) {
		...
		// 更新本次登录信息
		user.setLoginIp(StringUtils2.getRemoteAddr(Servlets.getRequest()));
	...
	public static String entryptPassword(String plainPassword) {
		...
		byte[] hashPassword = Digests.sha1(plain.getBytes(Charset.defaultCharset()), salt, HASH_INTERATIONS);
	...
	public void saveRole(Role role) throws ValidationException {
		if (StringUtils2.isBlank(role.getId())){
			role.preInsert();
			roleDao.insert(role);
			// 同步到Activiti
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public Boolean outUserInRole(Role role, User user) throws ValidationException {
	...
	@Transactional(readOnly = false)
	public User assignUserToRole(Role role, User user) throws ValidationException {
		if (user == null){
			return null;
		}
	...
	@Transactional(readOnly = false)
	public void saveMenu(Menu menu) throws ValidationException {
		...
		// 保存或更新实体
		if (StringUtils2.isBlank(menu.getId())){
			menu.preInsert();
			menuDao.insert(menu);
		}else{
			menu.preUpdate();
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	private void saveActivitiGroup(Role role) {
		...
		if (StringUtils2.isNotBlank(role.getOldEnname()) && !role.getOldEnname().equals(role.getEnname())){
		...
		for (User e : userList){
			String userId = e.getLoginName();//ObjectUtils2.toString(user.getId());
			// 如果该用户不存在，则创建一个
			...
			if (activitiUser == null){
				...
				activitiUser.setLastName(StringUtils2.EMPTY);
				activitiUser.setEmail(e.getEmail());
				activitiUser.setPassword(StringUtils2.EMPTY);
	...
	private void saveActivitiUser(User user) {
		...
		String userId = user.getLoginName();//ObjectUtils2.toString(user.getId());
		org.activiti.engine.identity.User activitiUser = identityService.createUserQuery().userId(userId).singleResult();
		if (activitiUser == null) {
			activitiUser = identityService.newUser(userId);
		}
