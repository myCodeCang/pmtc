...
public class Page<T> {
	...
	private int lengthView = 8;// 显示页面长度
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
		...
		if(StringUtils2.isBlank(size)){
			size =  request.getParameter("length");
		}
		...
		if(StringUtils2.isBlank(no)){
			String start = request.getParameter("start");
			if(StringUtils2.isNotBlank(start)){
				no = String.valueOf (Integer.parseInt(start)/pageSize + 1);
			}
	...
	public String toString() {
		...
		if (pageNo == first) {// 如果是首页
			sb.append("<li class=\"disabled am-disabled\"><a href=\"javascript:\">&#171; 上一页</a></li>\n");
		} else {
			sb.append("<li><a href=\"javascript:\" onclick=\""+funcName+"("+prev+","+pageSize+",'"+funcParam+"');\">&#171; 上一页</a></li>\n");
		}

		int begin = pageNo - (lengthView / 2);

		if (begin < first) {
			begin = first;
		}

		int end = begin + lengthView - 1;

		if (end >= last) {
			end = last;
			begin = end - lengthView + 1;
			if (begin < first) {
				begin = first;
			}
		}
		...
		if (begin > first) {
			...
			if (i < begin) {
				sb.append("<li class=\"disabled am-disabled\"><a href=\"javascript:\">...</a></li>\n");
			}
		}

		for (int i = begin; i <= end; i++) {
			if (i == pageNo) {
				sb.append("<li class=\"active am-active\"><a href=\"javascript:\">" + (i + 1 - first)
						+ "</a></li>\n");
			} else {
				sb.append("<li><a href=\"javascript:\" onclick=\""+funcName+"("+i+","+pageSize+",'"+funcParam+"');\">"
						+ (i + 1 - first) + "</a></li>\n");
		...
		if (pageNo == last) {
			sb.append("<li class=\"disabled am-disabled\"><a href=\"javascript:\">下一页 &#187;</a></li>\n");
		} else {
		...
		sb.insert(0,"<ul class=\"am-pagination am-pagination-centered\" >\n").append("</ul>\n");
...
//	}
	...
	public void setStart(int start) {
		this.start = start;
		this.pageNo = this.start/this.pageNo + 1;
	}
...
//		if (orderBy!=null){
//			for (String order : StringUtils2.split(orderBy, ",")){
//				String[] o = StringUtils2.split(order, " ");
//				if (o.length==1){
