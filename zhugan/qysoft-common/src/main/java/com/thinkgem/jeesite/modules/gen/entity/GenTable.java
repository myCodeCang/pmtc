...
public class GenTable extends DataEntity<GenTable> {
	...
	public String getParentTable() {
		return StringUtils2.lowerCase(parentTable);
	}
	...
	public String getParentTableFk() {
		return StringUtils2.lowerCase(parentTableFk);
	}
	...
	public List<String> getImportList(){
		...
		for (GenTableColumn column : getColumnList()){
			if (column.getIsNotBaseField() || ("1".equals(column.getIsQuery()) && "between".equals(column.getQueryType())
				...
				if (StringUtils2.indexOf(column.getJavaType(), ".") != -1 && !importList.contains(column.getJavaType())){
			...
			if (column.getIsNotBaseField()){
				...
				for (String ann : column.getAnnotationList()){
					if (!importList.contains(StringUtils2.substringBeforeLast(ann, "("))){
						importList.add(StringUtils2.substringBeforeLast(ann, "("));
					}
				}
			}
		}
	...
	public Boolean getParentExists(){
		return parent != null && StringUtils2.isNotBlank(parentTable) && StringUtils2.isNotBlank(parentTableFk);
	}
