...
public class ActTaskService extends BaseService {
	...
	public List<Act> todoList(Act act){
		String userId = UserUtils.getUser().getLoginName();//ObjectUtils2.toString(UserUtils.getUser().getId());
		...
		// 设置查询条件
		if (StringUtils2.isNotBlank(act.getProcDefKey())){
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
	...
	public List<Act> histoicFlowList(String procInsId, String startAct, String endAct){
		...
		for (int i=0; i<list.size(); i++){
			...
			// 过滤开始节点前的节点
			if (StringUtils2.isNotBlank(startAct) && startAct.equals(histIns.getActivityId())){
				start = true;
			}
			if (StringUtils2.isNotBlank(startAct) && !start){
				continue;
			}
			
			// 只显示开始节点和结束节点，并且执行人不为空的任务
			if (StringUtils2.isNotBlank(histIns.getAssignee())
...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
					if (il.size() > 0){
						if (StringUtils2.isNotBlank(il.get(0).getStartUserId())){
				...
				// 获取任务执行人名称
				if (StringUtils2.isNotEmpty(histIns.getAssignee())){
				...
				// 获取意见评论内容
				if (StringUtils2.isNotBlank(histIns.getTaskId())){
			...
			// 过滤结束节点后的节点
			if (StringUtils2.isNotBlank(endAct) && endAct.equals(histIns.getActivityId())){
				boolean bl = false;
				Integer actNum = actMap.get(histIns.getActivityId());
				...
				for (int j=i+1; j<list.size(); j++){
					...
					if ((actNumA != null && actNumA < actNum) || StringUtils2.equals(hi.getActivityId(), histIns.getActivityId())){
						bl = true;
					}
				}
				if (!bl){
	...
	public Page<Object[]> processList(Page<Object[]> page, String category) {
	    ...
	    if (StringUtils2.isNotEmpty(category)){
	...
	public String getFormKey(String procDefId, String taskDefKey){
		String formKey = "";
		if (StringUtils2.isNotBlank(procDefId)){
			if (StringUtils2.isNotBlank(taskDefKey)){
				try{
			...
			if (StringUtils2.isBlank(formKey)){
	...
	public String startProcess(String procDefKey, String businessTable, String businessId, String title, Map<String, Object> vars) {
		...
		// 设置流程标题
		if (StringUtils2.isNotBlank(title)){
	...
	public void complete(String taskId, String procInsId, String comment, String title, Map<String, Object> vars){
		// 添加意见
		if (StringUtils2.isNotBlank(procInsId) && StringUtils2.isNotBlank(comment)){
