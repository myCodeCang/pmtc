...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/user/userMailmessage/rece">收件箱列表</a></li>
	...
	<form:form id="searchForm" modelAttribute="userMailmessage" action="${ctx}/user/userMailmessage/rece" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<ul class="ul-form">
			<li><label>接收人：</label>
				<form:input path="toMemberName" htmlEscape="false" maxlength="255" class="input-medium"/>
			</li>
			<li><label>消息标题：</label>
			...
			<li class="btns"><input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/><input id="btnSubmit" class="btn btn-primary" type="reset" value="重置" onclick="window.location.href='${ctx}/user/userMailmessage/rece';" /></li>
		...
		<c:forEach items="${page.list}" var="userMailmessage">
			<tr>
				...
				<shiro:hasPermission name="user:userMailmessage:edit"><td>
					...
					<a href="${ctx}/user/userMailmessage/recedelete?id=${userMailmessage.id}" onclick="return confirmx('确认要删除该站内信吗？', this.href)">删除</a>
