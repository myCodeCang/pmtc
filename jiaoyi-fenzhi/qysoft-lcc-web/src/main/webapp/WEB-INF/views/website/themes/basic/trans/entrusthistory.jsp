...
<!-- 头部结束 -->
							...
							<table class="table table_striped align_right">
								<thead>
									<tr>
										...
										<th class="align_center">交易类型</th>
										<th class="align_center">会员编号</th>
										<th class="align_center">时间</th>
										<th class="align_center">委托数量</th>
										<th class="align_center">匹配数量</th>
								...
								<c:forEach items="${page.list}" var="transcodeBuy">
									<tr class="table_crumbs">
										...
										<td  class="align_center">
												${transcodeBuy.sellNum - transcodeBuy.downNum}
										</td>
										<td  class="align_center">
												${transcodeBuy.money}
										</td>
		...
		<script type="text/javascript">
            function page(pageNo,pageSize){
                window.location.href="${ctx}/trans/entrusthistory?siteBar=23&pageNo="+pageNo+"&pageSize="+pageSize;
            }
			$("a.xm-delete").on("click", function(evt) {
				evt.preventDefault();
				var keyWord = $(this).attr("title");
				var $link = $(this).attr("href");

				swal({
					title: '确定要' + keyWord + '该项吗?',
					text: "你将无法恢复！",
					type: "warning",
					showCancelButton: true,
					confirmButtonColor: "#3085d6",
					cancelButtonColor: '#d33',
					confirmButtonText: '是的,确定要' + keyWord,
					cancelButtonText: '不了，再想想',
					closeOnConfirm: false,
					allowOutsideClick: false
				}, function() {
					$.getJSON($link, function(dat) {
						var modalType = 'error';
						if(dat.status == 0) {
							modalType = 'success';
							setTimeout(function() {
